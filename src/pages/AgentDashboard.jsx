    ...
    <div className="app" dir="rtl">
      ...
      <div className="hero">
        <div className="hero-row">
          <div><div className="hero-name">שלום, {agentName}</div><div className="hero-date">{new Date().toLocaleDateString('he-IL',{weekday:'long',month:'long',day:'numeric'})}</div></div>
          <button className="exit-btn" onClick={()=>supabase.auth.signOut()}>⎋</button>
        </div>
        {agent && <>
          <div className="annual-block">
            <div className="annual-num">{fmtK(total)}</div>
            <div className="annual-sub">מתוך {fmtK(annual)} שנתי · נשארו {mLeft} חודשים</div>
            <div className="prog-track"><div className="prog-fill" style={{width:annPct+'%'}}/></div>
          </div>
          <div className="monthly-row">
            <div><div className="m-lbl">יעד החודש</div><div className="m-val">{fmtK(monthly)}</div></div>
            <div style={{textAlign:'left'}}><div className="m-lbl">הושג</div><div className="m-val">{fmt(won)}</div></div>
            <div style={{textAlign:'left'}}><div className="m-lbl">פגישות היום</div><div className="m-val" style={{color:todayM.length>0?'white':'rgba(255,255,255,.4)'}}>{todayM.length}</div></div>
          </div>
        </>}
      </div>
      ...
      {tab==='home' && <div className="body">
        {todayM.length>0 && (
          <div className="today-section">
            <div className="today-lbl">📅 פגישות היום</div>
            {todayM.map(l=>(
              <div key={l.id} className="today-row" onClick={()=>setTab('meetings')}>
                <div>
                  <div className="today-addr">{l.project_address}</div>
                  <div className="today-client">{l.client_name}</div>
                </div>
        ...
        {stale.length>0 && <>
          <div className="section-lbl urgent">⚠️ דורש טיפול</div>
          {stale.map(l=><LeadCard key={l.id} lead={l} onUpdate={load} onSchedule={setSchedLead}/>)}
        </>}

        {fresh.length>0 && <>
          <div className="section-lbl">לידים פעילים</div>
        ...
        {leads.length===0 && (
          <div className="empty">
            ...
            <button className="add-btn" style={{maxWidth:220,margin:'0 auto',display:'block'}} onClick={()=>setShowAdd(true)}>+ שיחה נכנסת חדשה</button>
          </div>
        )}
        {leads.length>0 && <button className="add-btn" onClick={()=>setShowAdd(true)}>+ שיחה נכנסת חדשה</button>}
      </div>}
      ...
      {tab==='leads' && <div className="body">
        <input className="search-input" placeholder="🔍 חפש כתובת, שם, טלפון" value={search} onChange={e=>setSearch(e.target.value)}/>
      ...
      <nav className="bottom-nav">
        <button className={`nav-btn ${tab==='home'?'on':''}`} onClick={()=>setTab('home')}>
          <i className="ti ti-home nav-icon" aria-hidden="true"/>בית
        </button>
        ...
        <button className={`nav-btn ${tab==='leads'?'on':''}`} onClick={()=>setTab('leads')}>
          <i className="ti ti-list nav-icon" aria-hidden="true"/>לידים
        </button>
        <button className="nav-btn" onClick={()=>setShowAdd(true)}>
          <i className="ti ti-plus nav-icon" aria-hidden="true"/>הוסף
        </button>
      </nav>
