  ...
  return (
    <div style={{ position: 'relative' }}>
      <input
        ...
        style={{ width:'100%', padding:'11px 13px', borderRadius:10, border:'.5px solid var(--line)', background:'var(--input)', color:'var(--ink)', fontSize:14, fontFamily:'inherit', textAlign:'left', direction:'ltr' }}
      />
        ...
        <div style={{ position:'absolute', top:'100%', right:0, left:0, zIndex:300, background:'#fff', borderRadius:12, border:'.5px solid var(--line)', boxShadow:'0 4px 16px rgba(0,0,0,.12)', overflow:'hidden', marginTop:4 }}>
          {suggestions.map(s => (
            <div key={s.place_id} onMouseDown={() => select(s)}
              style={{ padding:'10px 14px', fontSize:13, cursor:'pointer', borderBottom:'.5px solid var(--input)', color:'var(--ink)', display:'flex', alignItems:'flex-start', gap:8, direction:'ltr' }}
            >
              <span style={{ fontSize:16, flexShrink:0 }}>📍</span>
              <div>
                <div style={{ fontWeight:500 }}>{s.structured_formatting?.main_text}</div>
                <div style={{ fontSize:11, color:'var(--ink3)', marginTop:1 }}>{s.structured_formatting?.secondary_text}</div>
              </div>
            </div>
          ))}
        </div>
